...
Open this repository in IBM Bob and add the content of `src/bob-skills/port-operations.md` as a project skill, or attach it to a chat. Ask: `Use the port-operations skill to review the current HarborLens plan and draft a BLUF shift brief. Cite only data visible in the app or source.` Bob can inspect the deterministic calculation in `src/app.js` before drafting its response.
...
| Port 8000 is busy | Run `python src/server.py --port 8080` and open `http://localhost:8080`. |
